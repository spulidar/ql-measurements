...
This script reads Level 0 NetCDF files generated by LIBIDS, applies
instrumental corrections channel-by-channel, propagates statistical uncertainty,
estimates Planetary Boundary Layer height, integrates radiosonde thermodynamics,
calculates tropopause diagnostics, and exports a Level 1 RCS NetCDF product.
...
def _get_logger_from_args(args: tuple[Any, ...]) -> logging.Logger | None:
    """Extract a logger from process arguments when available."""
    if len(args) >= 3:
        return args[2]
    return globals().get("logger")


def _finite_or_fill(value: Any, fill_value: float = -999.0) -> float:
    """Convert a numeric value to float, replacing NaN/Inf/None by a fill value."""
    try:
        value = float(value)
...
def _get_channel_constant(
    ...
    Expected YAML format:
        ...
        physics:
          channels:
            "532.PC": [deadtime_us, bin_shift, background_offset]
    """
    if ch_name not in channels_config:
        logger.warning(
            f"  -> Channel {ch_name} is missing from physics.channels. "
            "Using neutral correction constants."
        )
...
def load_and_prepare_level0(
    ...
    The altitude coordinate is kept in meters throughout Level 1 because the
    physical corrections and the future Level 2 Rayleigh/KFS retrieval require
    consistent SI geometry.
    """
    try:
...
def apply_all_physical_corrections(
    ...
    For each channel this routine retrieves the raw signal, background range,
    channel-specific constants, optional dark-current profile, and then calls
    the physics kernel that returns corrected signal and Range Corrected Signal.
    """
    ...
    for ch_idx, ch_name in enumerate(ds.channel.values.astype(str)):
        ...
        try:
            ...
            if int(bg_mask.sum().values) < 2:
                logger.warning(
                    f"  -> Channel {ch_name}: background mask has fewer than 2 bins "
                    f"({bg_low:.1f}-{bg_high:.1f} m)."
                )
            ...
            if "Background_Profile" in ds:
                ...
                if dc_data.sizes.get("time_bck", 0) > 0:
                    ...
                    dc_err = (
                        dc_data.std(dim="time_bck", skipna=True).rename({"altitude": "range"})
                        / np.sqrt(max(ds.sizes.get("time_bck", 1), 1))
                    )
            ...
            ch_ds = xr.Dataset(
                {
                    "corrected_signal": (
                        rcs_c.rename({"range": "altitude"})
                        .assign_coords(channel=ch_name)
                        .astype(np.float32)
                    ),
                    "corrected_signal_error": (
                        err_rcs_c.rename({"range": "altitude"})
                        .assign_coords(channel=ch_name)
                        .astype(np.float32)
                    ),
                }
    ...
    final_ds["corrected_signal_error"].attrs.update(
        {
            "long_name": "One-sigma uncertainty of range corrected lidar signal",
            "units": "a.u. m^2",
        }
    )
    ...
    final_ds["channel_correction_success"].attrs.update(
        {"flag_values": "0, 1", "flag_meanings": "failed success"}
    )
    final_ds["dark_current_used"].attrs.update(
        {"flag_values": "0, 1", "flag_meanings": "not_used used"}
    )
...
def estimate_pbl_timeseries(
    ...
    try:
        ...
        logger.info(
            f"  -> Tracking PBL using {pbl_channel} "
            f"({min_search_m:.0f}-{max_search_m:.0f} m)."
        )
...
def integrate_thermodynamics(
    ...
    Pressure and temperature are stored for Level 2 because Rayleigh molecular
    backscatter/extinction depends directly on molecular number density.
    """
    try:
        dt_utc = pd.to_datetime(final_ds.time.values[len(final_ds.time) // 2])

        station_id = str(
            config.get("radiosonde", {}).get(
                "station_id",
                config.get("location", {}).get("station_id", "83779"),
            )
        )
        ...
        df_radio = (
            df_radio.dropna(subset=["height", "temperature", "pressure"])
        ...
        final_ds = final_ds.assign_coords(
            radiosonde_altitude=("radiosonde_altitude", df_radio["height"].values.astype(np.float64))
        )
        final_ds["radiosonde_altitude"].attrs.update(
            {"units": "m", "long_name": "Radiosonde altitude above mean sea level"}
        )

        final_ds["Radiosonde_Temperature_K"] = (
            ("radiosonde_altitude",),
            (df_radio["temperature"].values.astype(np.float64) + 273.15).astype(np.float32),
        )
        final_ds["Radiosonde_Temperature_K"].attrs.update(
            {
                "units": "K",
                "long_name": "Radiosonde air temperature",
                "source": "Wyoming Upper Air sounding",
            }
        )

        final_ds["Radiosonde_Pressure_hPa"] = (
            ("radiosonde_altitude",),
            df_radio["pressure"].values.astype(np.float32),
        )
        final_ds["Radiosonde_Pressure_hPa"].attrs.update(
            {
                "units": "hPa",
                "long_name": "Radiosonde atmospheric pressure",
                "source": "Wyoming Upper Air sounding",
            }
        )
...
def process_single_file(
    args: tuple[str | Path, Mapping[str, Any]] | tuple[str | Path, Mapping[str, Any], logging.Logger],
) -> str:
    """Process one Level 0 NetCDF into a Level 1 RCS NetCDF product."""
    if len(args) < 2:
        raise ValueError("process_single_file expects at least (nc_path, config).")

    nc_path, config = args[0], args[1]
    local_logger = _get_logger_from_args(args)

    if local_logger is None:
        raise RuntimeError("No logger was provided and no module-level logger exists.")
    ...
    try:
        ...
        save_path = (
            ...
            / stem[4:6]
            / stem
            / f"{stem}_level1_rcs.nc"
        )

        local_logger.info(f"[{stem}] Initializing Level 1 processing...")

        ds_raw, z_arr = load_and_prepare_level0(nc_file, local_logger)
        final_ds = apply_all_physical_corrections(ds_raw, z_arr, config, local_logger)
        final_ds = estimate_pbl_timeseries(final_ds, z_arr, config, local_logger)
        final_ds = integrate_thermodynamics(final_ds, config, local_logger)

        final_ds.attrs.update(ds_raw.attrs)
        final_ds.attrs.update(
            {
                "Processing_level": (
                    "Level 1: PC->MHz, DeadTime, Dark Current, Bin Shift, "
                    "Background subtraction, RCS, uncertainty propagation, PBL, Radiosonde, Tropopause"
                ),
...
if __name__ == "__main__":
    ...
    if not files:
        ...
        for f in files:
            logger.info(process_single_file((str(f), conf, logger)))
